...
- The replica bucket is in a near "read-only" mode, meaning you can only LIST objects. When the Offsite Replication option is used and in the event of a primary region unavailability/loss for your source bucket, the replica bucket will automatically switch to a read/write mode.
- Objects stored in replica bucket are billed differently. View pricing on [this page](/links/public-cloud/prices).

#### Q: What are the differences between the asynchronous replication feature and the Offsite Replication feature available in 3-AZ regions?
The Offsite Replication option offered in 3-AZ regions is based on the asynchronous replication feature. However, on one hand with Offsite Replication, OVHcloud is in charge of auto-generating a replication rule configuration with its own parameters where with the S3-compatible asynchronous replication, the customer has the full control over the feature (configuration and deployment).

#### Q: What if a replica bucket is deleted?
The source objects that are to be replicated will have a replication status FAILED when Object Storage try to replicate them but you will not get any errors.

#### Q: What about the initial replication rule?
The intial rule will remain unchanged and active until you disable it.

#### Q: What if a source bucket with offsite replication enabled is deleted?
Your replica bucket still exists but no more objects are replicated to it from your original source bucket.

#### Q: What about the initial replication rule?
The initial replication rule is set up on the source bucket and thus will also be deleted.

#### Q: Where will be stored the replicated data as the replication rule configuration is managed by OVHcloud?
Your replicated data is stored as any other data and will be stored in a bucket automatically generated by OVHcloud in a region of our chosing. The system will automatically determine the most suitable location among Strasbourg, Gravelines, and Roubaix, ensuring efficient and reliable offsite data protection.

#### Q: Can I modify the replication rule configuration?
...
#### Q: What is the name of the replica bucket?
The destination bucket name follows the pattern backup-{src-region}-{dst-region}-{timestamp_in_ms}-{src-bucket}

#### Q: How can I access my backed up data?
By default, the replica bucket is in a "near read-only" mode this means you can only LIST the objects inside the replica bucket. In the event of a primary region unavailability/loss for your source bucket, the replica bucket will automatically switch to a read/write mode and becomes available for additional requests.

#### Q: Which users/credentials can be used to acess the destination bucket?
When you activated the offsite replication during the source bucket creation, the same user that you associated with your source bucket will be associated with the replica bucket.

#### Q: How will my backup data be billed?
